            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if pygame.mouse.get_pressed() == (1,0,0):
                    ...
                    if grille.set_case(x,y,nb_coups%2+1):
                        ...
                        if grille.victoire(nb_coups%2+1,x,y,surf): # Ecran de victoire de l'humain si dif = 1 ou 2 / Ecran de victoire d'un des deux joueurs si dif = 0
                            pygame.display.flip()
                            sleep(0.4)
                            run=message_victoire("joueur %s"%(str(nb_coups%2+1),),grille,dif,surf,img_o,img_x)

                        nb_coups+=1
        
        if run and dif>=1 and nb_coups%2==0 and nb_coups-coup_debut<9:
            coord=tour_ia(dif, grille)
            if grille.victoire(1,coord[0],coord[1],surf):
                run = message_victoire("Robot",grille,dif,surf,img_o,img_x)
                    
            nb_coups+=1
        if run and nb_coups-coup_debut>=9:  # Ecran de fin si aucun gagnant
